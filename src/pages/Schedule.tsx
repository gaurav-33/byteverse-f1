    ...
    events: [
      {
        ...
        desc: "Registration window opens.",
    ...
    events: [
      {
        ...
        time: "TBA",
        title: "Info Session",
        desc: "Info session for participants.",
    ...
    events: [
      {
        ...
        desc: "Hackathon concludes.",
        location: "EVENT CAMPUS & ONLINE",
        type: "race",
        icon: "check_circle",
      },
      {
      ...
      },
      {
        ...
        desc: "Community review starts.",
        location: "COMMUNITY PORTAL",
        type: "event",
        icon: "chat",
      },
    ],
              ...
              <div key={day.day} className="relative">
                ...
                <div className="space-y-12">
                  {day.events.map((event, i) => {
                    ...
                    const previousEventsCount = scheduleData
                      .slice(0, dayIndex)
...
const TimelineEvent = ({
  ...
  return (
    <div
      className={`relative flex flex-col md:flex-row items-center w-full group overflow-visible`}
    >
      {/* Left Column (Desktop Only) */}
      <div
        className={`hidden md:flex w-full md:w-1/2 flex-col md:items-end md:text-right mb-8 md:mb-0 md:pr-12 lg:pr-16 ${!isLeft ? "md:opacity-100" : ""}`}
      >
        {isLeft ? (
          <EventCard
            event={event}
      ...
      >
        <div
          className={`absolute inset-0 rounded-full ${textColorClass.replace("text-", "bg-")} opacity-30 animate-ping -z-10`}
        ></div>
        <img
          src="/f1-stop.png"
          alt="Stop"
          className="w-full h-full object-contain drop-shadow-[0_0_10px_rgba(255,30,0,0.8)]"
        />
      </div>

      {/* Right Column (Mobile Content & Desktop Right) */}
      <div
        className={`w-full md:w-1/2 flex flex-col md:items-start md:text-left pl-36 md:pl-12 lg:pl-16`}
      >
...
const TimeDisplay = ({
  ...
  <div
    className={`hidden md:block w-full ${align === "right" ? "text-right" : "text-left"}`}
  >
    <div
      className={`font-mono text-2xl lg:text-3xl font-bold ${textColorClass} drop-shadow-[0_0_5px_rgba(0,0,0,0.5)]`}
    >
      {time}
    </div>
...
const EventCard = ({
  ...
  <div
    className={`relative w-full bg-white/5 border-l-2 ${borderColorClass} p-6 rounded-r-xl md:rounded-xl overflow-hidden group-hover:bg-white/10 transition-colors duration-300 hover:shadow-[0_0_30px_rgba(0,0,0,0.3)] text-left`}
  >
    ...
    {/* Hover Glow Gradient */}
    <div
      className={`absolute inset-0 bg-gradient-to-r from-${borderColorClass.replace("border-", "")}/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
    ></div>

    <div className="relative z-10">
      <div className={`flex items-center gap-3 mb-2 justify-start`}>
        <span className={`material-icons text-lg ${textColorClass}`}>
          {event.icon}
        </span>
        <span
          className={`text-[10px] font-bold uppercase tracking-widest border border-white/10 px-2 py-0.5 rounded ${textColorClass} bg-black/30`}
        >
      ...
      <p
        className={`text-sm text-gray-400 font-mono mb-4 border-l-2 border-white/10 pl-3`}
      >
        {event.desc}
      </p>

      <div
        className={`flex items-center gap-2 text-xs text-gray-500 font-mono uppercase tracking-wider justify-start`}
      >
